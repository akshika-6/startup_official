// src/pages/Startups.jsx - Adjustments for Navbar and Sidebar
...
import { Search, SlidersHorizontal, XCircle, Menu } from 'lucide-react'; // Ensure Menu is imported
...
const Startups = () => {
    ...
    return (
        <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            {/* Main Content Area */}
            {/* The md:ml-64 ensures content shifts when sidebar is open on medium+ screens */}
            <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-0'}`}>
                {/* Navbar */}
                <Navbar toggleSidebar={toggleSidebar} /> {/* Navbar is now at the top-right */}

                {/* Main page content */}
                {/* pt-16 accounts for the Navbar's height (h-16) */}
                <main className="relative z-10 flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 pt-16">
                    {/* Sidebar Toggle Button for the main content area */}
                    <div className="absolute top-4 left-4 md:hidden z-50"> {/* Only visible on medium screens down */}
                        <button
                            onClick={toggleSidebar}
                            className="p-2 rounded-full text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow"
