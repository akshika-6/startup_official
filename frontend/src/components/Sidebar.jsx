...
const Sidebar = ({ isOpen, toggleSidebar, sidebarWidthMd = "64" }) => {
  ...
  const investorNavItems = [
    { name: "Home", icon: HomeIcon, path: "/home-dashboard" },
    { name: "Dashboard", icon: LayoutDashboard, path: "/dashboard" },
    { name: "My Investments", icon: Briefcase, path: "/investments" },
    { name: "Submit Interest", icon: DollarSign, path: "/investor-deck" },
            ...
            <div className="space-y-1">
              {currentNavItems.map((item) => {
                ...
                ${
                  ...
                  >
                    <div
                      className={`flex items-center justify-center w-8 h-8 rounded-lg ${isActive ? "bg-white/20" : "group-hover:bg-white/10"}`}
                    >
